...
def generate_pdf_report(data: Dict[str, Any], logs: bool = False, output_path: str = "") -> None:
    ...
    if isinstance(src, list) and src:
        ...
        for item in src:
            ...
            parts.append(f"- {label}{suffix}: MD5={md5} | SHA1={sha1}")
        if parts:
            cs_para = f"{html_escape(L['checksums'])}:<br/>" + "<br/>".join([html_escape(p) for p in parts])
    ...
    meta_rows = [
        ...
        *([[Paragraph(cs_para, body_style)]] if cs_para else []),
