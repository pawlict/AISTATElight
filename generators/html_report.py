...
def _ui_labels(ui_lang: str) -> Dict[str, str]:
    ...
    if ui_lang.startswith("en"):
        return {
            ...
            "checksums": "Checksums",
            "cs_audio": "Audio",
            "cs_transcription": "Transcription",
            "cs_diarization": "Diarization",
        }
    return {
        "author": "Autor",
        ...
        "cs_audio": "Audio",
        "cs_transcription": "Transkrypcja",
        "cs_diarization": "Diaryzacja",
    }


def _checksum_label(L: Dict[str, str], kind: str) -> str:
    kind = (kind or "").lower()
    if kind == "audio":
        return L.get("cs_audio", "Audio")
    if kind == "transcription":
        return L.get("cs_transcription", "Transcription")
    if kind == "diarization":
        return L.get("cs_diarization", "Diarization")
    return kind or ""
...
def generate_html_report(data: Dict[str, Any], logs: bool = False, output_path: str = "report.html") -> None:
    ...
    # Checksums (audio + text payloads used for the report)
    cs_html = ""
    src = data.get("source_checksums") or []
    if isinstance(src, list) and src:
        rows: List[str] = []
        rows.append(
            f"<div class='meta-line'><span class='k'>{_esc(L['checksums'])}:</span></div>"
        )
        for item in src:
            ...
            if not (md5 or sha1):
                continue
            label = _checksum_label(L, kind)
            suffix = f" ({_esc(name)})" if name else ""
            rows.append(
                f"<div class='meta-line'><span class='k'>- {_esc(label)}{suffix}:</span> MD5={_esc(md5)} | SHA1={_esc(sha1)}</div>"
            )
        cs_html = "\n".join(rows)
