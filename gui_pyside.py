...
class MainWindow(QMainWindow):
    ...
    def _play_pending_hover(self) -> None:
        ...
        # Play the whole hovered segment (from start to end).
        start_ms = int(max(0.0, seg.start_s) * 1000)
        seg_len_ms = int(max(0.0, (seg.end_s - seg.start_s)) * 1000)
        play_len_ms = max(250, seg_len_ms if seg_len_ms > 0 else 2500)
    ...
    def _build_ui(self) -> None:
        ...
        # Content splitters (input/output + speaker names)
        ...
        # Give most vertical space to the transcription/diarization split view.
        self.splitter_v.setStretchFactor(0, 5)
        self.splitter_v.setStretchFactor(1, 1)
        self.splitter_v.setSizes([650, 170])
    ...
    def _build_menu(self) -> None:
        """Create actions for file operations (menu bar intentionally disabled).

        The app used to show a top menu "Plik". Per UI request we keep the actions
        (for reuse + translations) but do NOT attach them to a visible QMenuBar.
        """
        # Hidden container menu (not added to a QMenuBar).
        ...
        # Ensure the top "Plik" menu bar is not shown (tabs remain visible).
        try:
