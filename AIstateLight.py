...
# Dla PyTorch 2.6+: przywróć domyślne zachowanie torch.load(weights_only=False),
# żeby pyannote mógł wczytać checkpointy bez błędów "Weights only load failed".
...
class DiarizationApp(tk.Tk):
    ...
    def _create_main_tab(self, parent):
        # --- Górny panel: diarizacja tekstu ---
        settings_frame = ttk.LabelFrame(parent, text="Diarizacja tekstu (na bazie transkryptu)")
        ...
        audio_frame = ttk.LabelFrame(
            parent,
            text="Audio → transkrypcja (Whisper) i diarizacja po głosie (pyannote)",
        )
        audio_frame.pack(side=tk.TOP, fill=tk.X, padx=10, pady=5)

        ttk.Label(audio_frame, text="Język (puste = auto, np. 'pl'):").pack(side=tk.LEFT, padx=5)
        whisper_lang_entry = ttk.Entry(audio_frame, textvariable=self.whisper_lang_var, width=6)
        whisper_lang_entry.pack(side=tk.LEFT, padx=5)

        load_audio_btn = ttk.Button(
            audio_frame,
            text="Wczytaj plik audio (.m4a)",
        ...
        diarize_voice_btn = ttk.Button(
            audio_frame,
            text="Transkrybuj + diarizacja po głosie (Whisper + pyannote)",
        ...
        ttk.Label(
            right_frame,
            text="Wynik diarizacji tekstu / diarizacji po głosie:",
        ).pack(anchor="w")
    ...
    def _create_settings_tab(self, parent):
        # --- Ustawienia Whispera ---
        whisper_frame = ttk.LabelFrame(parent, text="Transkrypcja – model Whisper (jakość vs szybkość)")
        whisper_frame.pack(side=tk.TOP, fill=tk.X, padx=10, pady=5)

        ttk.Label(whisper_frame, text="Model Whisper:").pack(side=tk.LEFT, padx=5)
        ...
        # --- HF token dla pyannote ---
        pyannote_frame = ttk.LabelFrame(parent, text="Diarizacja po głosie – token HuggingFace (pyannote)")
        pyannote_frame.pack(side=tk.TOP, fill=tk.X, padx=10, pady=5)

        ttk.Label(pyannote_frame, text="HF token (pyannote):").pack(side=tk.LEFT, padx=5)
        ...
        ttk.Label(
            pyannote_frame,
            text="Token będzie zapisany w ~/.pyannote_hf_token i używany przy diarizacji.",
        ).pack(side=tk.LEFT, padx=5)
    ...
    def _create_menu(self):
        ...
        filemenu.add_command(label="Zapisz wynik diarizacji...", command=self.save_output)
    ...
    def update_status_label(self):
        ...
        text_method_display = self.method_var.get()
        text_method_code = self.get_current_text_diarization_code()
    ...
    def on_method_change(self, event=None):
        ...
        parent = self.speakers_label.master
    ...
    def on_whisper_model_change(self, event=None):
        # wymusimy przeładowanie modelu przy następnym użyciu
        self.current_whisper_model_name = None
        self.update_status_label()
    ...
    def on_transcribe_audio_clicked(self):
        if not self.audio_path:
            messagebox.showwarning("Brak audio", "Najpierw wybierz plik audio (.m4a).")
            return
        ...
        lang = self.whisper_lang_var.get().strip() or None
