...
_SPK_PREFIX_RE = re.compile(
    r"^(?P<spk>[A-Za-z0-9_][A-Za-z0-9_ \-]{0,31}):\s*(?P<rest>.*)$"
)

_SPK_AFTER_TS_RE = re.compile(
    r"^\s*(?P<spk>[A-Za-z0-9_][A-Za-z0-9_ \-]{0,31}):\s*(?P<txt>.*)$"
)
...
class SegmentTextEdit(QTextEdit):
    ...
    def _highlight_block(self, block_number: Optional[int]) -> None:
        # Clear / set a soft highlight on the whole line
        sels = []
    ...
    def apply(self) -> Dict[str, str]:
        ...
        def _apply_to_text(text: str) -> str:
            ...
            for old, new in mapping.items():
                ...
                out = re.sub(rf"(^\s*){re.escape(old)}(?=:)", rf"\\1{new}", out, flags=re.MULTILINE)
                out = re.sub(rf"(\]\s*){re.escape(old)}(?=:)", rf"\\1{new}", out)
            return out
