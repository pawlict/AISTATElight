...
class SegmentTextEdit(QTextEdit):
    ...
    def _speaker_tint(self, index: int):
        ...
        h = (hue + (index * 37)) % 360
        sat = max(40, min(120, hi.saturation() if hi.saturation() >= 0 else 90))
        val = max(200, min(255, (hi.value() if hi.value() >= 0 else 230) + 25))
        tint = QColor.fromHsv(int(h), int(sat), int(val))
        # Subtle blend so it is not "jaskrawe" and works in dark/light themes.
        return self._blend(base, tint, 0.14)
    ...
    def apply(self) -> Dict[str, str]:
        ...
        def _apply_to_text(text: str) -> str:
            ...
            for old, new in mapping.items():
                # Replace only speaker labels (line-start or right after timestamp bracket).
                # Keep any indentation / spacing captured in group(1).
                # Also allow optional spaces before ':' (some outputs have 'SPEAKER_00 : ...').
                pat1 = rf"(^\s*){re.escape(old)}(?=\s*:)"
                out = re.sub(pat1, lambda m: (m.group(1) or "") + new, out, flags=re.MULTILINE)

                pat2 = rf"(\]\s*){re.escape(old)}(?=\s*:)"
                out = re.sub(pat2, lambda m: (m.group(1) or "") + new, out)
            return out

        self.left.setPlainText(_apply_to_text(self.left.toPlainText()))
        self.right.setPlainText(_apply_to_text(self.right.toPlainText()))

        # Recompute per-speaker coloring (diarization view) after global renaming.
        try:
            self.left._maybe_recompute_speaker_coloring()
        except Exception:
            pass
        try:
            self.right._maybe_recompute_speaker_coloring()
        except Exception:
            pass
        self.refresh()
